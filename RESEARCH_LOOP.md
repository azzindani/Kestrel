...
10b. **PHASE-2 STAGING MAINTENANCE (owner directive 2026-06-23 — every firing).** Staging is the
    curated **best-performers** pool: a SEPARATE compose project (`kestrel-staging`, ENV=staging,
    `STAGING_ENGINE=sim` until BingX VST keys exist) that shares the labs postgres + Grafana, rows
    isolated by env='staging' + the `staging-` bot_id prefix. Its Grafana dashboard
    ...
    ```
    # 1. re-rank dev performers by EXPECTANCY, regenerate the staging fleet (falls back to the
    #    lockbox leads macd_cross/macd_rsi when no dev cell is +EV yet):
    docker compose exec -T kestrel python3 scripts/promote_to_staging.py --stdout > bots.staging.json.new
    ...
    if ! diff -q bots.staging.json bots.staging.json.new; then
      ...
      docker compose -p kestrel-staging -f docker-compose.staging.yml up -d   # load refreshed fleet
