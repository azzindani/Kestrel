...
TH_PF = {"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 1}, {"color": "green", "value": 1.5}]}
...
def stat(title, w, h, sql, unit=None, thresholds=None, color="value", graph="none", decimals=None):
    panels.append({"datasource": DS, "fieldConfig": fc(unit, thresholds, decimals=decimals), "gridPos": place(w, h), "id": nid(),
...
def ts(title, w, h, sql, unit="currencyUSD", fmt="table", bars=False, fill=20, color=None, step=False, mx=None, mn=None, tline=None, stack=False):
...
def histogram(title, w, h, sql, unit=None, color="blue", bucket=None):
    ...
    f["defaults"]["custom"] = {"fillOpacity": 70, "gradientMode": "none", "hideFrom": {"legend": False, "tooltip": False, "viz": False}, "lineWidth": 1}
    opts = {"bucketOffset": 0, "combine": False, "legend": {"calcs": [], "displayMode": "list", "placement": "bottom", "showLegend": False},
            "tooltip": {"mode": "single", "sort": "none"}}
    if bucket is not None:
        opts["bucketSize"] = bucket
    panels.append({"datasource": DS, "fieldConfig": f, "gridPos": place(w, h), "id": nid(),
                   "options": opts, "pluginVersion": PV, "targets": [tgt(sql)], "title": title, "type": "histogram"})
...
def col_override(col, unit, bg=True, th=None):
    ...
    return {"matcher": {"id": "byName", "options": col},
            "properties": [{"id": "unit", "value": unit}, {"id": "custom.cellOptions", "value": cell}, {"id": "thresholds", "value": th or TH_PNL}]}
...
stat("Return on Capital", 4, 4, "SELECT COALESCE(SUM(pnl_net_usdt),0) AS roi FROM trades WHERE exit_ts IS NOT NULL", "percent", TH_PNL, decimals=2)
stat("Open Positions", 4, 4, "SELECT COUNT(*) AS open FROM trades WHERE exit_ts IS NULL", thresholds=TH_BLUE)
gauge("Today PnL vs −$5 limit (UTC)", 4, 4, "SELECT COALESCE(SUM(pnl_net_usdt),0) AS today FROM trades WHERE exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) >= date_trunc('day', NOW() AT TIME ZONE 'UTC')", "currencyUSD", TH_LIMIT, mn=-5, mx=5)
...
stat("Win Rate (closed)", 3, 4, "SELECT CASE WHEN COUNT(*)=0 THEN 0 ELSE 100.0*SUM((pnl_net_usdt>0)::int)/COUNT(*) END AS win_rate FROM trades WHERE exit_ts IS NOT NULL", "percent", TH_WIN, decimals=1)
stat("Expectancy ($/trade)", 3, 4, "SELECT COALESCE(AVG(pnl_net_usdt),0) AS v FROM trades WHERE exit_ts IS NOT NULL", "currencyUSD", TH_PNL, decimals=4)
stat("Profit Factor", 3, 4, "SELECT COALESCE(SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt>0)/NULLIF(-SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt<=0),0),0) AS v FROM trades WHERE exit_ts IS NOT NULL", thresholds=TH_PF, decimals=2)
stat("Avg Realized R", 3, 4, f"SELECT ROUND(AVG((CASE WHEN direction='long' THEN exit_price-entry_price ELSE entry_price-exit_price END)/NULLIF(ABS(entry_price-sl_price),0)),3) AS v {_CLOSED}", thresholds=TH_PNL, decimals=3)
stat("Best Trade", 3, 4, "SELECT COALESCE(MAX(pnl_net_usdt),0) AS v FROM trades WHERE exit_ts IS NOT NULL", "currencyUSD", TH_GREEN, decimals=4)
stat("Worst Trade", 3, 4, "SELECT COALESCE(MIN(pnl_net_usdt),0) AS v FROM trades WHERE exit_ts IS NOT NULL", "currencyUSD", TH_RED, decimals=4)
stat("Capital Deployed (open margin)", 3, 4, "SELECT COALESCE(SUM(size_usdt),0) AS v FROM trades WHERE exit_ts IS NULL", "currencyUSD", TH_BLUE)
...
ts("Drawdown (equity − peak, from $100 start)", 12, 7,
   ...
   color="red", fill=40)
ts("Underwater % (drawdown as % of peak)", 12, 7,
   ...
   unit="percent", color="orange", fill=40)
stat("Max Drawdown", 6, 4,
...
stat("Current Drawdown", 6, 4,
     ...
     "currencyUSD", TH_DD)
stat("Peak Equity", 6, 4, f"SELECT GREATEST(COALESCE(MAX(equity),100),100) AS v FROM ({_EQUITY}) q", "currencyUSD", TH_GREEN)
stat("Best Day (UTC)", 3, 4, "SELECT COALESCE(MAX(d),0) AS v FROM (SELECT SUM(pnl_net_usdt) d FROM trades WHERE exit_ts IS NOT NULL GROUP BY (exit_ts/86400000)) x", "currencyUSD", TH_GREEN, decimals=4)
stat("Worst Day (UTC)", 3, 4, "SELECT COALESCE(MIN(d),0) AS v FROM (SELECT SUM(pnl_net_usdt) d FROM trades WHERE exit_ts IS NOT NULL GROUP BY (exit_ts/86400000)) x", "currencyUSD", TH_RED, decimals=4)
ts("Net PnL per Trade", 24, 6,
...
ts("Daily Net PnL (UTC)", 12, 7,
   "SELECT to_timestamp((exit_ts/86400000)*86400) AS \"time\", SUM(pnl_net_usdt) AS daily_pnl FROM trades WHERE exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) BETWEEN $__timeFrom() AND $__timeTo() GROUP BY (exit_ts/86400000) ORDER BY 1", bars=True, fill=70)
ts("Rolling Avg PnL — last 20 trades", 12, 7,
   "SELECT to_timestamp(exit_ts/1000.0) AS \"time\", AVG(pnl_net_usdt) OVER (ORDER BY exit_ts ROWS BETWEEN 19 PRECEDING AND CURRENT ROW) AS rolling_avg_pnl FROM trades WHERE exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) BETWEEN $__timeFrom() AND $__timeTo() ORDER BY exit_ts", tline=0)
...
histogram("Realized R-multiple — distribution", 8, 8, f"SELECT (CASE WHEN direction='long' THEN exit_price-entry_price ELSE entry_price-exit_price END)/NULLIF(ABS(entry_price-sl_price),0) AS r_multiple {_CLOSED}", color="green")
...
table("PnL Percentiles ($/trade)", 12, 8,
...
bargauge("Avg Win vs Avg Loss ($)", 12, 8,
         f"SELECT 'Avg Win' AS metric, ROUND(AVG(pnl_net_usdt) FILTER (WHERE pnl_net_usdt>0),4) AS value {_CLOSED} UNION ALL SELECT 'Avg Loss', ROUND(AVG(pnl_net_usdt) FILTER (WHERE pnl_net_usdt<=0),4) {_CLOSED}", "currencyUSD", TH_PNL)
table("🥇 Best 10 Trades", 12, 8,
...
ts("Rolling Win-Rate — last 20 vs cumulative (55% = go-live target)", 24, 7,
...
bargauge("Win Rate by Direction", 8, 7, "SELECT direction AS metric, ROUND(100.0*AVG((pnl_net_usdt>0)::int),1) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY direction ORDER BY value DESC", "percent", TH_WIN)
bargauge("Win Rate by Pair", 8, 7, "SELECT pair AS metric, ROUND(100.0*AVG((pnl_net_usdt>0)::int),1) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY pair ORDER BY value DESC", "percent", TH_WIN)
bargauge("Win Rate by Hour-of-Day (UTC)", 8, 7, f"SELECT lpad(({_HOUR})::text,2,'0')||':00' AS metric, ROUND(100.0*AVG((pnl_net_usdt>0)::int),1) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY {_HOUR} ORDER BY {_HOUR}", "percent", TH_WIN)
table("Win Rate by Confidence Band (signal→trade)", 12, 8,
...
table("Win Rate by Regime (signal→trade)", 12, 8,
...
_STREAK = ("SELECT t.pnl_net_usdt>0 AS win, ROW_NUMBER() OVER (ORDER BY t.exit_ts) "
           "- ROW_NUMBER() OVER (PARTITION BY t.pnl_net_usdt>0 ORDER BY t.exit_ts) AS grp "
           "FROM trades t WHERE t.exit_ts IS NOT NULL")
stat("Max Consecutive Losses", 4, 4, f"SELECT COALESCE(MAX(c),0) AS v FROM (SELECT COUNT(*) c FROM ({_STREAK}) s WHERE win=false GROUP BY grp) z", thresholds=TH_RED)
stat("Max Consecutive Wins", 4, 4, f"SELECT COALESCE(MAX(c),0) AS v FROM (SELECT COUNT(*) c FROM ({_STREAK}) s WHERE win=true GROUP BY grp) z", thresholds=TH_GREEN)
stat("Profit Factor", 4, 4, "SELECT COALESCE(SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt>0)/NULLIF(-SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt<=0),0),0) AS v FROM trades WHERE exit_ts IS NOT NULL", thresholds=TH_PF, decimals=2)
stat("Payoff Ratio (avg win / avg loss)", 4, 4, "SELECT COALESCE(AVG(pnl_net_usdt) FILTER (WHERE pnl_net_usdt>0)/NULLIF(-AVG(pnl_net_usdt) FILTER (WHERE pnl_net_usdt<=0),0),0) AS v FROM trades WHERE exit_ts IS NOT NULL", thresholds=TH_PF, decimals=2)
stat("Expectancy in R", 4, 4, f"SELECT ROUND(AVG((CASE WHEN direction='long' THEN exit_price-entry_price ELSE entry_price-exit_price END)/NULLIF(ABS(entry_price-sl_price),0)),3) AS v {_CLOSED}", thresholds=TH_PNL, decimals=3)
stat("Largest Loss as % of Capital", 4, 4, "SELECT ROUND(ABS(LEAST(MIN(pnl_net_usdt),0))/100.0*100,3) AS v FROM trades WHERE exit_ts IS NOT NULL", "percent", TH_DD, decimals=3)
bargauge("Realized R-multiple Distribution", 12, 8,
...
stat("TP-hit Rate", 4, 4, f"SELECT 100.0*AVG((close_reason='take_profit')::int) AS v {_CLOSED}", "percent", TH_WIN, decimals=1)
stat("SL-hit Rate", 4, 4, f"SELECT 100.0*AVG((close_reason='stop_loss')::int) AS v {_CLOSED}", "percent", TH_BLUE, decimals=1)
stat("Timeout Rate", 4, 4, f"SELECT 100.0*AVG((close_reason='timeout')::int) AS v {_CLOSED}", "percent", TH_BLUE, decimals=1)
stat("Liquidation Rate", 4, 4, f"SELECT 100.0*AVG((close_reason='liquidated')::int) AS v {_CLOSED}", "percent", TH_ZERO_BAD, decimals=1)
...
stat("Avg MFE % (best excursion)", 4, 4, f"SELECT ROUND(AVG({_MFE}),3) AS v FROM trades t {_PX} WHERE t.exit_ts IS NOT NULL", "percent", TH_GREEN)
stat("Avg MAE % (worst excursion)", 4, 4, f"SELECT ROUND(AVG({_MAE}),3) AS v FROM trades t {_PX} WHERE t.exit_ts IS NOT NULL", "percent", TH_RED)
...
histogram("MFE % — distribution", 8, 7, f"SELECT {_MFE} AS mfe_pct FROM trades t {_PX} WHERE t.exit_ts IS NOT NULL", "percent", color="green")
histogram("MAE % — distribution", 8, 7, f"SELECT {_MAE} AS mae_pct FROM trades t {_PX} WHERE t.exit_ts IS NOT NULL", "percent", color="red")
table("Excursion by Close Reason — did winners run / losers get stopped on noise?", 24, 8,
      ...
      overrides=[col_override("avg_mfe_pct", "percent", bg=False), col_override("avg_mae_pct", "percent", bg=False), col_override("avg_realized_r", "short"), col_override("net_pnl", "currencyUSD")])
...
stat("Signals Rejected (events)", 4, 4, "SELECT COUNT(*) AS v FROM events WHERE category='signal' AND message LIKE 'signal_rejected:%'", thresholds=TH_BLUE)
stat("Risk Rejections", 4, 4, "SELECT COUNT(*) AS v FROM events WHERE category='risk' AND message LIKE 'risk_rejected%'", thresholds=TH_BLUE)
stat("Orders Placed", 4, 4, "SELECT COUNT(*) AS v FROM events WHERE category='order' AND message LIKE 'order_placed%'", thresholds=TH_GREEN)
stat("Fire Rate (fired / evaluated)", 4, 4, "SELECT ROUND(100.0*(SELECT COUNT(*) FROM signals WHERE outcome='fired')::numeric / NULLIF((SELECT COUNT(*) FROM signals WHERE outcome='fired') + (SELECT COUNT(*) FROM events WHERE category='signal' AND message LIKE 'signal_rejected:%'),0),3) AS v", "percent", TH_BLUE, decimals=3)
stat("Avg Confidence (fired)", 4, 4, "SELECT ROUND(AVG(confidence),3) AS v FROM signals WHERE outcome='fired'", thresholds=TH_BLUE, decimals=3)
bargauge("Rejection Funnel — by reason (all-time)", 12, 8,
...
bargauge("Signal-stage Layer Pass-rate (fired signals)", 12, 8,
...
ts("Signals Over Time — fired vs rejected (hourly)", 24, 7,
   ...
   unit="short", fmt="time_series", bars=True, fill=60)
table("Fired Signals — recent (pattern · confidence · regime · layers)", 24, 7,
...
table("PnL by Pattern", 8, 8, "SELECT pattern, COUNT(*) FILTER (WHERE exit_ts IS NOT NULL) AS closed, ROUND(100.0*SUM((pnl_net_usdt>0)::int)/NULLIF(COUNT(*) FILTER (WHERE exit_ts IS NOT NULL),0),1) AS win_rate, ROUND(AVG(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS avg_pnl, ROUND(SUM(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS net_pnl FROM trades GROUP BY pattern ORDER BY net_pnl NULLS LAST",
      overrides=[col_override("net_pnl", "currencyUSD"), col_override("avg_pnl", "currencyUSD"), col_override("win_rate", "percent", bg=False, th=TH_WIN)])
bargauge("Signals Fired by Pattern", 8, 8, "SELECT pattern AS metric, COUNT(*) AS value FROM signals WHERE outcome='fired' GROUP BY pattern ORDER BY value DESC")
histogram("Signal Confidence — distribution (fired)", 8, 8, "SELECT confidence FROM signals WHERE outcome='fired'", color="purple")
table("Pattern Memory (learned win-rates by pattern · dir · session · regime)", 24, 8,
      ...
      overrides=[col_override("win_rate", "percent", bg=False, th=TH_WIN), col_override("avg_pnl_pct", "percent", bg=False)])

# ════════════════════════════════════════════════════════════════════════════
# 9 — Regime Analytics  (regime lives on signals; joined to trades for PnL)
# ════════════════════════════════════════════════════════════════════════════
section("🌊 Regime Analytics")
pie("Signals by Regime", 8, 8, "SELECT regime AS metric, COUNT(*) AS value FROM signals GROUP BY regime ORDER BY value DESC")
bargauge("Net PnL by Regime (signal→trade)", 8, 8, "SELECT s.regime AS metric, ROUND(SUM(t.pnl_net_usdt),4) AS value FROM signals s JOIN trades t ON s.trade_id=t.id WHERE t.exit_ts IS NOT NULL GROUP BY s.regime ORDER BY value", "currencyUSD", TH_PNL)
table("Regime Performance (signal→trade)", 8, 8, "SELECT s.regime, COUNT(t.id) AS trades, ROUND(100.0*AVG((t.pnl_net_usdt>0)::int),1) AS win_rate, ROUND(AVG(t.pnl_net_usdt),4) AS avg_pnl, ROUND(SUM(t.pnl_net_usdt),4) AS net_pnl FROM signals s JOIN trades t ON s.trade_id=t.id WHERE t.exit_ts IS NOT NULL GROUP BY s.regime ORDER BY net_pnl NULLS LAST",
      overrides=[col_override("net_pnl", "currencyUSD"), col_override("avg_pnl", "currencyUSD"), col_override("win_rate", "percent", bg=False, th=TH_WIN)])
...
table("Current Market Snapshot (latest candle per pair)", 24, 9,
      ...
      overrides=[col_override("rsi14", "short", bg=False, th={"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 70}, {"color": "red", "value": 80}]}),
                 col_override("adx", "short", bg=False, th={"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 15}, {"color": "green", "value": 20}]})])
ts("RSI(14) Over Time — per pair (70/30 bands)", 12, 8,
   ...
   unit="short", fmt="time_series", fill=0, mx=100, mn=0)
ts("ATR % (volatility, pair-comparable) Over Time", 12, 8,
   ...
   unit="percent", fmt="time_series", fill=0)
ts("ADX(14) Over Time — per pair (20 = trend threshold)", 12, 8,
   ...
   unit="short", fmt="time_series", fill=0, tline=20)
ts("Volume Ratio Over Time — per pair (1.0 = average)", 12, 8,
   ...
   unit="short", fmt="time_series", fill=0, tline=1)
pie("Candle Direction Balance (up vs down)", 8, 7, "SELECT direction AS metric, COUNT(*) AS value FROM (SELECT DISTINCT ON (pair,ts) pair, ts, direction FROM candles WHERE direction IS NOT NULL ORDER BY pair, ts) x GROUP BY direction")
bargauge("Avg ATR % (volatility) by Pair", 8, 7, "SELECT pair AS metric, ROUND(AVG(atr14/NULLIF(close,0)*100),3) AS value FROM candles WHERE atr14 IS NOT NULL GROUP BY pair ORDER BY value DESC", "percent", TH_BLUE)
bargauge("Avg ADX by Pair (trend strength)", 8, 7, "SELECT pair AS metric, ROUND(AVG(adx),1) AS value FROM candles WHERE adx IS NOT NULL GROUP BY pair ORDER BY value DESC", "short", {"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 15}, {"color": "green", "value": 20}]})

# ════════════════════════════════════════════════════════════════════════════
# 11 — HP-Tuning Lab (grid: TP×SL×hold)
# ════════════════════════════════════════════════════════════════════════════
section("🧪 HP-Tuning Lab (grid: TP×SL×hold)")
table("🏆 Strategy Leaderboard — by net PnL (each row = one grid cell)", 24, 9,
      ...
      overrides=[col_override("net_pnl", "currencyUSD"), col_override("avg_pnl", "currencyUSD"), col_override("win_rate", "percent", bg=False, th=TH_WIN)])
bargauge("Avg net PnL/trade by TP multiple", 8, 7,
         f"SELECT 'TP '||round((regexp_match({_TOK},'t([0-9]+)'))[1]::numeric/10,1) AS metric, ROUND(AVG(pnl_net_usdt),5) AS value FROM trades WHERE {_LAB} GROUP BY 1 ORDER BY metric", "currencyUSD", TH_PNL)
bargauge("Avg net PnL/trade by SL multiple", 8, 7,
         f"SELECT 'SL '||round((regexp_match({_TOK},'s([0-9]+)'))[1]::numeric/10,1) AS metric, ROUND(AVG(pnl_net_usdt),5) AS value FROM trades WHERE {_LAB} GROUP BY 1 ORDER BY metric", "currencyUSD", TH_PNL)
bargauge("Avg net PnL/trade by max-hold", 8, 7,
         f"SELECT 'hold '||(regexp_match({_TOK},'h([0-9]+)'))[1] AS metric, ROUND(AVG(pnl_net_usdt),5) AS value FROM trades WHERE {_LAB} GROUP BY 1 ORDER BY metric", "currencyUSD", TH_PNL)
table("TP × SL grid — avg net PnL/trade (rows=SL, cols=TP)", 12, 8,
      ...
      overrides=[col_override("TP 1.0", "currencyUSD"), col_override("TP 1.6", "currencyUSD"), col_override("TP 2.4", "currencyUSD"), col_override("TP 3.0", "currencyUSD")])
bargauge("Sample size (closed trades) per grid cell", 12, 8,
         f"SELECT {_TOK} AS metric, COUNT(*) AS value FROM trades WHERE {_LAB} GROUP BY 1 ORDER BY value DESC LIMIT 24",
         "short", {"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 10}, {"color": "green", "value": 30}]})

# ════════════════════════════════════════════════════════════════════════════
# 12 — Per-Bot / Pair Breakdown
# ════════════════════════════════════════════════════════════════════════════
section("🏆 Per-Bot & Per-Pair Breakdown")
table("Per-Bot Performance", 12, 8, "SELECT bot_id, COUNT(*) FILTER (WHERE exit_ts IS NOT NULL) AS closed, COUNT(*) FILTER (WHERE exit_ts IS NULL) AS open, ROUND(100.0*SUM((pnl_net_usdt>0)::int) FILTER (WHERE exit_ts IS NOT NULL)/NULLIF(COUNT(*) FILTER (WHERE exit_ts IS NOT NULL),0),1) AS win_rate, ROUND(SUM(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS net_pnl FROM trades GROUP BY bot_id ORDER BY net_pnl DESC NULLS LAST LIMIT 40",
      overrides=[col_override("net_pnl", "currencyUSD"), col_override("win_rate", "percent", bg=False, th=TH_WIN)])
table("PnL by Pair", 12, 8, "SELECT pair, COUNT(*) FILTER (WHERE exit_ts IS NOT NULL) AS closed, ROUND(100.0*SUM((pnl_net_usdt>0)::int) FILTER (WHERE exit_ts IS NOT NULL)/NULLIF(COUNT(*) FILTER (WHERE exit_ts IS NOT NULL),0),1) AS win_rate, ROUND(AVG(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS avg_pnl, ROUND(SUM(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS net_pnl FROM trades GROUP BY pair ORDER BY net_pnl NULLS LAST",
      overrides=[col_override("net_pnl", "currencyUSD"), col_override("avg_pnl", "currencyUSD"), col_override("win_rate", "percent", bg=False, th=TH_WIN)])
bargauge("Trades per Bot (activity)", 12, 7, "SELECT bot_id AS metric, COUNT(*) AS value FROM trades GROUP BY bot_id ORDER BY value DESC LIMIT 25", "short", TH_BLUE)
bargauge("Net PnL by Pair", 12, 7, "SELECT pair AS metric, ROUND(SUM(pnl_net_usdt) FILTER (WHERE exit_ts IS NOT NULL),4) AS value FROM trades GROUP BY pair ORDER BY value", "currencyUSD", TH_PNL)
...
bargauge("Net PnL by Session", 12, 8,
         f"SELECT session AS metric, ROUND(SUM(pnl_net_usdt),4) AS value FROM (SELECT pnl_net_usdt, {_SESSION_CASE} AS session FROM (SELECT pnl_net_usdt, {_HOUR} AS h FROM trades WHERE exit_ts IS NOT NULL) a) b GROUP BY session ORDER BY value", "currencyUSD", TH_PNL)
bargauge("Net PnL by Hour-of-Day (UTC)", 12, 7, f"SELECT lpad(({_HOUR})::text,2,'0')||':00' AS metric, ROUND(SUM(pnl_net_usdt),4) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY {_HOUR} ORDER BY {_HOUR}", "currencyUSD", TH_PNL)
bargauge("Trades by Hour-of-Day (UTC)", 12, 7, f"SELECT lpad(({_HOUR})::text,2,'0')||':00' AS metric, COUNT(*) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY {_HOUR} ORDER BY {_HOUR}", "short", TH_BLUE)
bargauge("Net PnL by Day-of-Week (UTC)", 12, 7, "SELECT to_char(to_timestamp(entry_ts/1000.0),'Dy') AS metric, ROUND(SUM(pnl_net_usdt),4) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY to_char(to_timestamp(entry_ts/1000.0),'Dy'), EXTRACT(ISODOW FROM to_timestamp(entry_ts/1000.0)) ORDER BY EXTRACT(ISODOW FROM to_timestamp(entry_ts/1000.0))", "currencyUSD", TH_PNL)
bargauge("Trades by Day-of-Week (UTC)", 12, 7, "SELECT to_char(to_timestamp(entry_ts/1000.0),'Dy') AS metric, COUNT(*) AS value FROM trades WHERE exit_ts IS NOT NULL GROUP BY to_char(to_timestamp(entry_ts/1000.0),'Dy'), EXTRACT(ISODOW FROM to_timestamp(entry_ts/1000.0)) ORDER BY EXTRACT(ISODOW FROM to_timestamp(entry_ts/1000.0))", "short", TH_BLUE)

# ════════════════════════════════════════════════════════════════════════════
# 14 — Fees & Costs
# ════════════════════════════════════════════════════════════════════════════
section("💸 Fees & Costs")
stat("Total Fees Paid", 4, 4, "SELECT ROUND(COALESCE(SUM(fee_entry_usdt+COALESCE(fee_exit_usdt,0)),0),4) AS v FROM trades", "currencyUSD", TH_RED)
stat("Avg Fee per Trade", 4, 4, f"SELECT ROUND(AVG(fee_entry_usdt+COALESCE(fee_exit_usdt,0)),4) AS v {_CLOSED}", "currencyUSD", TH_BLUE)
stat("Gross PnL (pre-fee)", 4, 4, f"SELECT ROUND(COALESCE(SUM(pnl_gross_usdt),0),4) AS v {_CLOSED}", "currencyUSD", TH_PNL)
stat("Net PnL (post-fee)", 4, 4, f"SELECT ROUND(COALESCE(SUM(pnl_net_usdt),0),4) AS v {_CLOSED}", "currencyUSD", TH_PNL)
stat("Fee Drag (fees / |gross|)", 4, 4, f"SELECT ROUND(100.0*SUM(fee_entry_usdt+COALESCE(fee_exit_usdt,0))/NULLIF(ABS(SUM(pnl_gross_usdt)),0),1) AS v {_CLOSED}", "percent", TH_RED, decimals=1)
stat("Avg Round-trip Cost (% of bucket)", 4, 4, f"SELECT ROUND(AVG((fee_entry_usdt+COALESCE(fee_exit_usdt,0))/NULLIF(size_usdt,0))*100,3) AS v {_CLOSED}", "percent", TH_RED, decimals=3)
ts("Cumulative Fees Paid Over Time", 12, 7, "SELECT to_timestamp(exit_ts/1000.0) AS \"time\", SUM(fee_entry_usdt+COALESCE(fee_exit_usdt,0)) OVER (ORDER BY exit_ts) AS cumulative_fees FROM trades WHERE exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) BETWEEN $__timeFrom() AND $__timeTo() ORDER BY exit_ts", color="red", fill=30)
ts("Gross vs Net PnL (cumulative)", 12, 7, "SELECT to_timestamp(exit_ts/1000.0) AS \"time\", SUM(pnl_gross_usdt) OVER (ORDER BY exit_ts) AS gross, SUM(pnl_net_usdt) OVER (ORDER BY exit_ts) AS net FROM trades WHERE exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) BETWEEN $__timeFrom() AND $__timeTo() ORDER BY exit_ts", fmt="time_series")
...
stat("Bots alive (<90s)", 6, 4, f"SELECT COUNT(DISTINCT bot_id) AS bots FROM heartbeats WHERE ts >= {_NOW_MS} - 90000",
     thresholds={"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 1}, {"color": "green", "value": 200}]})
stat("Heartbeat age (s)", 6, 4, "SELECT COALESCE(((EXTRACT(EPOCH FROM NOW())*1000)::BIGINT - MAX(ts))/1000.0, 9999) AS age FROM heartbeats", "s",
     {"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 60}, {"color": "red", "value": 90}]}, graph="area")
stat("Candles written (1h)", 6, 4, "SELECT COUNT(*) AS c FROM candles WHERE to_timestamp(ts/1000.0) >= NOW() - INTERVAL '1 hour'",
     thresholds={"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 1}, {"color": "green", "value": 50}]})
stat("Errors+Critical (24h)", 6, 4, f"SELECT COUNT(*) AS c FROM events WHERE level IN ('ERROR','CRITICAL') AND ts >= {_NOW_MS} - 86400000",
     thresholds={"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 1}, {"color": "red", "value": 5}]})
bargauge("Events by Category (24h)", 8, 7, f"SELECT category AS metric, COUNT(*) AS value FROM events WHERE ts >= {_NOW_MS} - 86400000 GROUP BY category ORDER BY value DESC")
pie("Events by Level (24h)", 8, 7, f"SELECT level AS metric, COUNT(*) AS value FROM events WHERE ts >= {_NOW_MS} - 86400000 GROUP BY level ORDER BY value DESC")
bargauge("Signal Rejections by Reason (24h)", 8, 7, f"SELECT split_part(message,':',2) AS metric, COUNT(*) AS value FROM events WHERE category='signal' AND message LIKE 'signal_rejected:%' AND ts >= {_NOW_MS} - 86400000 GROUP BY metric ORDER BY value DESC LIMIT 12")
ts("Event Rate Over Time (hourly, by category)", 24, 7,
   ...
   unit="short", fmt="time_series", bars=True, fill=50, stack=True)
table("Bot Heartbeats", 24, 8, f"SELECT bot_id, status, pid, to_timestamp(ts/1000.0) AS last_heartbeat, CASE WHEN ts >= {_NOW_MS} - 90000 THEN 1 ELSE 0 END AS alive FROM heartbeats ORDER BY bot_id LIMIT 250",
      overrides=[map_override("alive", {"0": {"color": "red", "index": 0, "text": "DOWN"}, "1": {"color": "green", "index": 1, "text": "UP"}})])
table("Recent Events", 24, 8, "SELECT to_timestamp(ts/1000.0) AS time, bot_id, level, category, message FROM events ORDER BY ts DESC LIMIT 50",
