...
class SweepRule:
    """One table's retention rule. `days` <= 0 disables the sweep for that table."""

    table: str
    days: int
    # SQL fragment ANDed into the delete predicate to protect linked rows (§19).
...
def load_rules(env: dict[str, str]) -> list[SweepRule]:
    ...
    return [
        SweepRule(
            ...
            "AND NOT EXISTS (SELECT 1 FROM trade_context tc WHERE tc.candle_id = t.id)",
        ),
        SweepRule("signals", int(env.get("SIGNALS_RETENTION_DAYS", "60"))),
        SweepRule("events", int(env.get("EVENTS_RETENTION_DAYS", "30"))),
        SweepRule("microstructure", int(env.get("MICROSTRUCTURE_RETENTION_DAYS", "365"))),
    ]
...
async def sweep_table(conn, rule: SweepRule, now_ms: int) -> dict[str, int]:
    ...
    guard = rule.guard_sql or ""
    sql = (
        f"DELETE FROM {rule.table} WHERE ctid IN ("
        f"  SELECT t.ctid FROM {rule.table} t WHERE t.ts < $1 {guard} LIMIT {_BATCH_ROWS}"
        f")"
    )
...
async def run_sweep(cfg: AppConfig, notifier: TelegramNotifier) -> None:
    ...
    async with db_conn.acquire() as conn:
        ...
        for rule in rules:
            results[rule.table] = await sweep_table(conn, rule, now_ms)
