...
def main() -> None:
    ...
    print(
        f"pairs={len(pairs)} variants={len(VARIANTS)} backtests={len(pairs) * len(VARIANTS)} "
    ...
    for pi, pair in enumerate(pairs, 1):
        ...
        for v in VARIANTS:
            ...
            trades = run_backtest(candles, p, cfg, bot_id=f"bt-{pair}-{v['name']}", enabled_patterns=v["patterns"])[
                "trades"
            ]
            ...
            print(
                f"        {v['name']:6s}: OOS n={mo['total_trades']} win={mo['win_rate'] * 100:.1f}% "
                f"avg=${mo['avg_pnl_usdt']:.4f} net=${mo['total_pnl_usdt']:.3f} "
                f"expR={mo['expectancy_R']:.3f}",
                flush=True,
            )
    ...
    print(
        f"  {'variant':8s} {'n':>5s} {'win%':>6s} {'avg$':>9s} {'net$':>9s} {'R/R':>5s} "
        f"{'expR':>6s} {'PF':>5s} {'tp%':>5s} {'sl%':>5s} {'to%':>5s} {'IS→OOS':>8s}",
        flush=True,
    )
    for r in table:
        m = r["oos"]
        ...
        print(
            f"  {r['name']:8s} {m['total_trades']:5d} {m['win_rate'] * 100:6.1f} {m['avg_pnl_usdt']:9.4f} "
    ...
    if table:
        ...
        print(
            f"  best: {table[0]['name']} avg=${b['avg_pnl_usdt']:.4f}/trade win={b['win_rate'] * 100:.1f}% "
            f"n={b['total_trades']} expR={b['expectancy_R']:.3f} — {verdict}",
            flush=True,
        )
    if skipped:
...
def _write_md(tag, args, cfg, pairs, table, passing, skipped) -> None:
    ...
    lines = [
        f"# Kestrel wave-strategy backtest — {tag} UTC",
        "",
    ...
    for r in table:
        ...
        lines.append(
            f"| {r['name']} | {r['patterns'][0]} | {prof} | {m['total_trades']} | "
    ...
    if passing:
        ...
        b = table[0]["oos"]
        lines.append(
            f"**No variant clears the §30 OOS bar.** Best was `{table[0]['name']}` at "
