...
def main() -> None:
    ...
    print(f"source={ex_used}  candles={len(candles)}  "
          f"grid={len(TP_GRID)*len(SL_GRID)*len(MC_GRID)*len(VOL_GRID)} configs", flush=True)
            ...
            for mc in MC_GRID:
                for vol in VOL_GRID:
                    ...
                    p = dataclasses.replace(
                        base, tp_atr_multiplier=tp, sl_atr_multiplier=sl,
                        min_confidence=mc, volume_ratio_min=vol, max_hold_candles=HOLD,
                    )
                    ...
                    results.append({
                        "net": m["total_pnl_usdt"], "win": m["win_rate"], "rr": rr,
                        "n": m["total_trades"], "tp": tp, "sl": sl, "mc": mc, "vol": vol,
                        "is_win": mi["win_rate"], "is_net": mi["total_pnl_usdt"],
                    })
                    print(f"[{i:2d}/{total}] tp={tp} sl={sl} mc={mc} vol={vol} -> "
                          f"OOS n={m['total_trades']:3d} win={m['win_rate']*100:5.1f}% "
                          f"net=${m['total_pnl_usdt']:8.3f} rr={rr:.2f}", flush=True)

    results.sort(key=lambda r: -r["net"])
    print("\n=== TOP 10 configs by OOS net PnL ===", flush=True)
    for r in results[:10]:
        print(f"  net=${r['net']:8.3f}  win={r['win']*100:5.1f}%  rr={r['rr']:4.2f}  n={r['n']:3d}  "
              f"| tp={r['tp']} sl={r['sl']} mc={r['mc']} vol={r['vol']}  "
              f"| IS win={r['is_win']*100:.1f}% net=${r['is_net']:.2f}", flush=True)
    ...
    print(f"  best OOS net = ${best['net']:.3f}  (win {best['win']*100:.1f}%, n={best['n']}) "
          f"at tp={best['tp']} sl={best['sl']} mc={best['mc']} vol={best['vol']}", flush=True)
    print(f"\n  >>> {'AT LEAST ONE CONFIG VALIDATES — investigate it' if passing else 'NO in-range config clears the bar — confirms the strategy has no edge'} <<<", flush=True)
