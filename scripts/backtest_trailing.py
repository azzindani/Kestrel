...
VARIANTS = [
    {
        ...
        "trail": {"tp_atr_multiplier": 3.0, "sl_atr_multiplier": 1.6, "max_hold_candles": 24,
    ...
    },
    {
        ...
        "trail": {"sl_atr_multiplier": 1.0, "max_hold_candles": 8,
...
def main() -> None:
    ...
    print(f"pairs={len(pairs)} variants={len(VARIANTS)} arms={len(ARMS)} "
          f"backtests={len(pairs) * len(VARIANTS) * len(ARMS)} cost/trade≈{bg._COST_PCT:.2f}%", flush=True)

    # pooled[(variant, arm)] = {oos, ins, full, n_oos_candles}
    pooled: dict[tuple[str, str], dict[str, list]] = {
        (v["name"], a): {"oos": [], "ins": [], "full": [], "n_oos_candles": 0}
    ...
    for pi, pair in enumerate(pairs, 1):
        ...
        for v in VARIANTS:
            ...
            print(f"        {v['name']:6s}: fixed avg=${mfx['avg_pnl_usdt']:.4f} "
                  f"| trail avg=${mtr['avg_pnl_usdt']:.4f}", flush=True)
    ...
    for v in VARIANTS:
        for arm in ARMS:
            ...
            rows.append({"name": v["name"], "arm": arm, "oos": mo, "ins": mi,
                         "degrade": round(mo["avg_pnl_usdt"] - mi["avg_pnl_usdt"], 5)})

    print("\n=== TRAILING A/B LEADERBOARD (out-of-sample) ===", flush=True)
    print(f"  {'variant':8s} {'arm':6s} {'n':>5s} {'win%':>6s} {'avg$':>9s} {'net$':>9s} "
          f"{'expR':>6s} {'PF':>5s} {'trail%':>6s} {'sl%':>5s} {'to%':>5s} {'IS→OOS':>8s}", flush=True)
    for v in VARIANTS:
        for arm in ARMS:
            ...
            trail_rate = (sum(1 for t in tr if t.get("close_reason") == "trailing_stop") / len(tr) * 100.0
                          if tr else 0.0)
            print(f"  {r['name']:8s} {arm:6s} {m['total_trades']:5d} {m['win_rate'] * 100:6.1f} "
    ...
    for v in VARIANTS:
        ...
        print(f"  {v['name']:6s}: fixed ${fx['avg_pnl_usdt']:.4f} → trail ${tr['avg_pnl_usdt']:.4f} "
              f"({delta:+.4f}/trade) — {'TRAILING BETTER' if better else 'fixed better'}", flush=True)
...
def _write_md(tag, args, cfg, pairs, rows, pooled, wins, skipped) -> None:
    ...
    lines = [f"# Kestrel trailing-close A/B backtest — {tag} UTC", "",
    ...
    for r in rows:
        ...
        trail_rate = (sum(1 for t in tr if t.get("close_reason") == "trailing_stop") / len(tr) * 100.0
                      if tr else 0.0)
        lines.append(f"| {r['name']} | {r['arm']} | {m['total_trades']} | {m['win_rate'] * 100:.1f} | "
