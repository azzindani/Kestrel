...
def _ext_metrics(trades: list[dict], n_candles: int) -> dict[str, Any]:
    ...
    if not trades:
        return {**base, "expectancy_R": 0.0, "sortino": 0.0, "calmar": 0.0,
                "median_pnl_usdt": 0.0, "max_consec_losses": 0, "max_consec_wins": 0,
                "tp_rate": 0.0, "sl_rate": 0.0, "timeout_rate": 0.0, "liq_rate": 0.0,
                "pct_clearing_cost": 0.0, "time_in_market_pct": 0.0, "avg_planned_RR": 0.0,
                "avg_mfe_pct": 0.0, "avg_mae_pct": 0.0, "avg_hold_minutes": 0.0}
    ...
    for t in chrono:
        if float(t["pnl_net_usdt"]) > 0:
            ...
            cur_l += 1; cur_w = 0
        mc_l = max(mc_l, cur_l); mc_w = max(mc_w, cur_w)
...
def main() -> None:
    ...
    print(f"pairs={len(pairs)}  cells={len(cells)} (TP×SL×hold = "
          f"{len(lab.TP_GRID)}×{len(lab.SL_GRID)}×{len(lab.HOLD_GRID)})  "
          f"backtests={len(pairs) * len(cells)}  cost/trade≈{_COST_PCT:.2f}%", flush=True)

    # rows[cell_token] accumulates per-cell pooled OOS/IS/full trade lists across pairs
    pooled: dict[str, dict[str, list]] = {
        lab._tok(tp, sl, h): {"oos": [], "ins": [], "full": [], "n_oos_candles": 0}
    ...
    for pi, pair in enumerate(pairs, 1):
        try:
            ...
            print(f"[{pi}/{len(pairs)}] {pair}: FETCH FAILED ({type(exc).__name__}: {str(exc)[:80]}) — skipped", flush=True)
            skipped.append(pair)
            continue
        ...
        print(f"[{pi}/{len(pairs)}] {pair}: src={ex_used} candles={len(candles)} "
              f"(OOS={n_oos_candles})", flush=True)
        ...
        for (tp, sl, h) in cells:
            ...
            pair_rows.append({"pair": pair, "cell": tok, "tp": tp, "sl": sl, "hold": h,
                              "oos_n": mo["total_trades"], "oos_win": mo["win_rate"],
                              "oos_net": mo["total_pnl_usdt"], "oos_avg": mo["avg_pnl_usdt"],
                              "oos_expR": mo["expectancy_R"], "oos_rr": _rr(mo)})
        ...
        b = pair_rows[0]
        print(f"        best cell {b['cell']}: OOS n={b['oos_n']} win={b['oos_win']*100:.1f}% "
              f"avg=${b['oos_avg']:.4f} net=${b['oos_net']:.3f} expR={b['oos_expR']:.3f}", flush=True)
    ...
    for tok, d in pooled.items():
        ...
        table.append({"cell": tok, "oos": mo, "ins": mi,
    ...
    passing = [r for r in table if r["oos"]["total_pnl_usdt"] > 0 and r["oos"]["win_rate"] > 0.55
               and _rr(r["oos"]) >= 1.2 and r["oos"]["total_trades"] >= 30]
...
def _print_leaderboard(table: list[dict]) -> None:
    ...
    print(f"  {'cell':12s} {'n':>5s} {'win%':>6s} {'avg$':>9s} {'net$':>9s} {'R/R':>5s} "
          f"{'expR':>6s} {'PF':>5s} {'shrp':>6s} {'srt':>6s} {'maxDD$':>8s} "
          f"{'tp%':>5s} {'sl%':>5s} {'to%':>5s} {'IS→OOS':>8s}", flush=True)
    for r in table:
        m = r["oos"]
        pf = m["profit_factor"] if m["profit_factor"] is not None else float("inf")
        print(f"  {r['cell']:12s} {m['total_trades']:5d} {m['win_rate']*100:6.1f} "
              ...
              f"{m['timeout_rate']*100:5.1f} {r['degrade']:+8.4f}", flush=True)
...
def _print_verdict(table: list[dict], passing: list[dict], skipped: list[str]) -> None:
    ...
    if doa:
        print(f"  DEAD cells (planned R/R = tp/sl < 1.2 risk floor → 0 trades, never fire): "
              f"{len(doa)} — {', '.join(doa)}", flush=True)
    if passing:
        for r in passing[:5]:
            m = r["oos"]
            print(f"    >>> {r['cell']}: win={m['win_rate']*100:.1f}% R/R={_rr(m):.2f} "
                  f"net=${m['total_pnl_usdt']:.3f} n={m['total_trades']} — VALIDATES, investigate", flush=True)
    else:
        print(f"  best cell {table[0]['cell'] if table else '—'}: avg=${best['avg_pnl_usdt']:.4f}/trade "
              f"win={best['win_rate']*100:.1f}% n={best['total_trades']} — "
              f"NO cell clears the bar (confirms no edge in this grid on real data)", flush=True)
    if skipped:
        print(f"  NOTE: {len(skipped)} pair(s) skipped (fetch failed): {', '.join(skipped)} — "
              f"leaderboard excludes them.", flush=True)


def _flat(cell: str, tag: str, m: dict, sample: str) -> dict:
    """One flat CSV row for a (cell, sample) — sample in {'oos','ins'}."""
    return {"cell": cell, "sample": sample, "n": m["total_trades"], "win_rate": m["win_rate"],
            "net_usdt": m["total_pnl_usdt"], "avg_usdt": m["avg_pnl_usdt"],
            "median_usdt": m["median_pnl_usdt"], "expectancy_R": m["expectancy_R"],
            "avg_planned_RR": m["avg_planned_RR"], "rr": round(_rr(m), 3),
            "profit_factor": m["profit_factor"], "sharpe": m["sharpe_ratio"],
            "sortino": m["sortino"], "calmar": m["calmar"], "max_dd_usdt": m["max_drawdown_usdt"],
            "max_dd_pct": m["max_drawdown_pct"], "max_consec_losses": m["max_consec_losses"],
            "tp_rate": m["tp_rate"], "sl_rate": m["sl_rate"], "timeout_rate": m["timeout_rate"],
            "liq_rate": m["liq_rate"], "pct_clearing_cost": m["pct_clearing_cost"],
            "time_in_market_pct": m["time_in_market_pct"], "avg_hold_candles": m["avg_hold_candles"],
            "avg_hold_minutes": m["avg_hold_minutes"], "avg_mfe_pct": m["avg_mfe_pct"],
...
def _write_reports(tag, args, cfg, pairs, cells, table, per_pair_best, pooled, passing, skipped) -> None:
    ...
    keep = {"pair", "direction", "pattern", "entry_ts", "exit_ts", "hold_candles", "entry_price",
            "exit_price", "tp_price", "sl_price", "close_reason", "pnl_net_usdt", "pnl_pct",
            "realized_R", "planned_RR", "mfe_pct", "mae_pct"}
    top_json = {}
    ...
    lines = [f"# Kestrel backtest grid sweep — {tag} UTC", "",
             f"- **window:** {args.days}d {args.tf} · **leverage:** {cfg.leverage}x · "
             f"**bucket:** ${cfg.bucket_size_usdt} · cost/trade ≈ {_COST_PCT:.2f}%",
             f"- **pairs:** {', '.join(pairs)}" + (f" · **skipped (fetch failed):** {', '.join(skipped)}" if skipped else ""),
    ...
    for r in table:
        ...
        lines.append(f"| {r['cell']} | {m['total_trades']} | {m['win_rate']*100:.1f} | "
    ...
    lines += ["", "## Best cell per pair (out-of-sample)", "",
              "| pair | best cell | n | win% | avg $ | net $ | expR | R/R |",
              "|---|---|--:|--:|--:|--:|--:|--:|"]
    for b in per_pair_best:
        lines.append(f"| {b['pair']} | {b['cell']} | {b['oos_n']} | {b['oos_win']*100:.1f} | "
                     f"{b['oos_avg']:.4f} | {b['oos_net']:.3f} | {b['oos_expR']:.2f} | {b['oos_rr']:.2f} |")
    doa = [r["cell"] for r in table if r["oos"]["total_trades"] == 0]
    lines += ["", "## Verdict", ""]
    if doa:
        lines.append(f"> **{len(doa)} grid cells are dead-on-arrival** (planned R/R = tp/sl < 1.2 risk "
    ...
    if passing:
        ...
        for r in passing:
            m = r["oos"]
            lines.append(f"- `{r['cell']}` — win {m['win_rate']*100:.1f}%, R/R {_rr(m):.2f}, "
                         f"net ${m['total_pnl_usdt']:.3f}, n={m['total_trades']}")
    else:
        b = table[0]["oos"] if table else {}
        lines.append(f"**No cell clears the §30 OOS bar.** Best was `{table[0]['cell'] if table else '—'}` "
