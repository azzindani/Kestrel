...
TH_PF = {"mode": "absolute", "steps": [{"color": "red", "value": None}, {"color": "orange", "value": 1}, {"color": "green", "value": 1.2}]}
...
TH_ERR = {"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 1}, {"color": "red", "value": 5}]}
# heartbeat / feed age in seconds: green fresh, red stale
TH_AGE = {"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 90}, {"color": "red", "value": 300}]}
...
TH_SLIP = {"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 1.15}, {"color": "red", "value": 2}]}
...
def histogram(title, w, h, sql, unit=None, color="blue"):
    ...
    panels.append({"datasource": DS, "fieldConfig": f, "gridPos": place(w, h), "id": nid(),
                   ...
                   "pluginVersion": PV, "targets": [tgt(sql)], "title": title, "type": "histogram"})
...
def table(title, w, h, sql, overrides=None):
    ...
    panels.append({"datasource": DS, "fieldConfig": f, "gridPos": place(w, h), "id": nid(),
                   ...
                   "pluginVersion": PV, "targets": [tgt(sql, "table")], "title": title, "type": "table"})
...
SLIP_LATERAL = ("LEFT JOIN LATERAL (SELECT close FROM candles WHERE pair=t.pair "
                "AND timeframe=t.timeframe AND env=t.env AND ts <= t.entry_ts "
                "ORDER BY ts DESC LIMIT 1) c ON true")
SLIP_EXPR = ("(CASE WHEN t.direction='long' THEN (t.entry_price - c.close) "
             "ELSE (c.close - t.entry_price) END) / NULLIF(c.close,0) * 100")
REALIZED_R = ("(CASE WHEN direction='long' THEN exit_price-entry_price ELSE entry_price-exit_price END)"
              "/NULLIF(ABS(entry_price-sl_price),0)")

# ── Panels ───────────────────────────────────────────────────────────────────
section("Quarantine Health — is the live demo actually up?")
stat("Live Bots (hb < 90s)", 4, 4, f"SELECT COUNT(*) AS v FROM heartbeats WHERE {ENV_HB} AND ts >= {NOW_MS} - 90000", thresholds=TH_UP, color="background")
stat("Configured Bots", 4, 4, f"SELECT COUNT(*) AS v FROM heartbeats WHERE {ENV_HB}", thresholds=TH_BLUE)
stat("Last Heartbeat Age (s)", 4, 4, f"SELECT COALESCE(({NOW_MS} - MAX(ts))/1000.0, 999999) AS v FROM heartbeats WHERE {ENV_HB}", "s", TH_AGE, decimals=0)
stat("Feed: Last Candle Age (min)", 4, 4, f"SELECT COALESCE(({NOW_MS} - MAX(ts))/60000.0, 999999) AS v FROM candles WHERE {ENV_T}", "m", {"mode": "absolute", "steps": [{"color": "green", "value": None}, {"color": "orange", "value": 250}, {"color": "red", "value": 600}]}, decimals=0)
stat("Connection Errors (24h)", 4, 4, f"SELECT COUNT(*) AS v FROM events WHERE {ENV_T} AND category='connection' AND level IN ('WARN','ERROR','CRITICAL') AND ts >= {NOW_MS} - 86400000", thresholds=TH_ERR, color="background")
...
stat("Orders Entered (24h)", 4, 4, f"SELECT COUNT(*) AS v FROM trades WHERE {ENV_T} AND entry_ts >= {NOW_MS} - 86400000", thresholds=TH_BLUE)
stat("Fills Closed (24h)", 4, 4, f"SELECT COUNT(*) AS v FROM trades WHERE {ENV_T} AND exit_ts >= {NOW_MS} - 86400000", thresholds=TH_BLUE)
stat("Order Errors (24h)", 4, 4, f"SELECT COUNT(*) AS v FROM events WHERE {ENV_T} AND category='order' AND level IN ('ERROR','CRITICAL') AND ts >= {NOW_MS} - 86400000", thresholds=TH_ERR, color="background")
stat("Risk Rejections (24h)", 4, 4, f"SELECT COUNT(*) AS v FROM signals WHERE {ENV_T} AND outcome='rejected' AND ts >= {NOW_MS} - 86400000", thresholds=TH_BLUE)
stat("Liquidations (all)", 4, 4, f"SELECT COUNT(*) AS v FROM trades WHERE {ENV_T} AND close_reason='liquidated'", thresholds=TH_ERR, color="background")
stat("Avg Fill→Close Hold (candles)", 4, 4, f"SELECT ROUND(AVG(hold_candles),1) AS v FROM trades {CLOSED}", thresholds=TH_BLUE, decimals=1)
table("Recent Order / Position / Connection Events", 24, 8, f"SELECT to_timestamp(ts/1000.0) AS time, bot_id, level, category, message FROM events WHERE {ENV_T} AND category IN ('order','position','connection') ORDER BY ts DESC LIMIT 30")

section("Realized Performance — selected phase ($env)")
stat("Account Balance (start $100)", 4, 4, f"SELECT 100 + COALESCE(SUM(pnl_net_usdt),0) AS v FROM trades {CLOSED}", "currencyUSD", TH_PNL, graph="area")
stat("Realized PnL", 4, 4, f"SELECT COALESCE(SUM(pnl_net_usdt),0) AS v FROM trades {CLOSED}", "currencyUSD", TH_PNL)
stat("Win Rate", 4, 4, f"SELECT CASE WHEN COUNT(*)=0 THEN 0 ELSE 100.0*SUM((pnl_net_usdt>0)::int)/COUNT(*) END AS v FROM trades {CLOSED}", "percent", TH_WIN, decimals=1)
stat("Expectancy ($/trade)", 4, 4, f"SELECT COALESCE(AVG(pnl_net_usdt),0) AS v FROM trades {CLOSED}", "currencyUSD", TH_PNL, decimals=4)
stat("Profit Factor", 4, 4, f"SELECT COALESCE(SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt>0)/NULLIF(-SUM(pnl_net_usdt) FILTER (WHERE pnl_net_usdt<=0),0),0) AS v FROM trades {CLOSED}", thresholds=TH_PF, decimals=2)
gauge("Today PnL vs −$5 limit (UTC)", 4, 4, f"SELECT COALESCE(SUM(pnl_net_usdt),0) AS v FROM trades WHERE {ENV_T} AND exit_ts IS NOT NULL AND to_timestamp(exit_ts/1000.0) >= date_trunc('day', NOW() AT TIME ZONE 'UTC')", "currencyUSD", TH_LIMIT, mn=-5, mx=5)
ts("Equity Curve ($100 + cumulative realized PnL)", 16, 8,
...
table("Per-Bot Leaderboard ($env)", 8, 8,
...
stat("Avg Entry Slippage % (signed)", 6, 4, f"SELECT ROUND(AVG({SLIP_EXPR}),4) AS v FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL", "percent", TH_PNL, decimals=4)
stat("Slippage vs 0.05% model (×)", 6, 4, f"SELECT ROUND(AVG(ABS({SLIP_EXPR}))/0.05, 2) AS v FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL", thresholds=TH_SLIP, color="background", decimals=2)
stat("Avg Abs Slippage %", 6, 4, f"SELECT ROUND(AVG(ABS({SLIP_EXPR})),4) AS v FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL", "percent", TH_BLUE, decimals=4)
stat("Trades Measured", 6, 4, f"SELECT COUNT(*) AS v FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL AND c.close IS NOT NULL", thresholds=TH_BLUE)
histogram("Entry slippage % — distribution (0.05% = model)", 12, 7, f"SELECT ROUND({SLIP_EXPR}, 4) AS slippage_pct FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL", "percent", color="orange")
ts("Entry slippage % over time (line = 0.05% model)", 12, 7, f"SELECT to_timestamp(t.entry_ts/1000.0) AS \"time\", {SLIP_EXPR} AS slippage_pct FROM trades t {SLIP_LATERAL} WHERE t.env='{ENVF}' AND t.exit_ts IS NOT NULL AND to_timestamp(t.entry_ts/1000.0) BETWEEN $__timeFrom() AND $__timeTo() ORDER BY t.entry_ts", unit="percent", color="orange", tline=0.05)

section("Labs ↔ Phase cross-check — does the demo venue match the sim?")
table("Same (pair, strategy) cells: dev vs $env", 24, 9,
...
dashboard = {
    ...
    "panels": panels, "refresh": "30s", "schemaVersion": 39, "tags": ["kestrel", "staging", "live-ops"],
    "templating": {"list": [
        {
            "name": "env",
            "label": "Phase",
    ...
    "timezone": "utc", "title": "Kestrel — Staging / Live Ops ($env)", "uid": "kestrel-staging",
    "version": 1, "weekStart": "",
}
