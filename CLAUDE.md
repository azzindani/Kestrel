...
```
**Portfolio guard ("manager bot" · engine/portfolio_guard.py · risk-shaping, ✗ edge):** one in-process coordinator sums unrealised PnL across the WHOLE fleet and force-closes EVERY bot's open positions (taker market-out · each bot closes its OWN, one-owner §11) when aggregate unrealised crosses ±threshold of total equity — `PORTFOLIO_DD_PCT` drawdown-stop / `PORTFOLIO_TP_PCT` profit-lock (0 = disabled, live-safe default). **STAGING-ONLY (2026-06-27, owner): both 0.10 in .env.staging · DISABLED in dev override (0) — in dev the guard is a forward-test CONFOUND (truncating trades before their natural strategy exit contaminates per-pattern edge stats); dev measures the RAW strategy, the overlay lives in the live-like layer.** FLEET-AGGREGATE, ✗ per-bucket (one bucket at −10% is the per-trade SL's job). It watches **real-time UNREALISED on open positions** (a correlated-crash circuit breaker) — **✗ cumulative realised drawdown** (so it never fired in dev's whole history; the dashboard's big down-number is booked realised loss, nothing open to close). The −DD half = legit crash insurance at 20×; the +TP half caps the right tail / fights let-winners-run (§22 trailing) — questionable. Risk-shaping + adds forced-close cost, ✗ creates edge; real bleed is per-trade stop_loss. For a CUMULATIVE equity-floor halt, that's a separate kill-switch (not built).
